...
class Campaigns {
    ...
    static function get_campaigns() {
        if(self::$campaigns === false) {
            $campaigns_raw = get_posts(array('post_type' => self::$name, 'posts_per_page'   => -1));
            ...
            foreach($campaigns_raw as $campaign) {
                ...
                $campaign->link = get_permalink($campaign->ID);
        ...
        foreach(self::$campaigns as $i => $campaign) {
            ...
            switch($type) {
                ...
                case 'donation box':
                    ...
                    $product = ProductManagement::get_product_data($campaign->ID);

                    $after = '<form action="'. home_url('store/cart/') .'" method="post">';
                    $after .= "<input data-prefix='$ ' class='col-md-4 margin-vert' type='text' name='item_user_price' value='25.00' />";
                    $after .= '<input type="hidden" value="addToCart" name="task">';
                    $after .= '<input type="hidden" value="' . $product['id'] . '" name="cart66ItemId">';
                    $after .= '<input type="hidden" value="'. $permalink .'" name="product_url">';
                    $after .= "<a href='{$permalink}' class='margin-vert learn-more-link'>Learn More About {$campaign->post_title} <i class='fa fa-external-link'></i></a>";
                    $after .= "<button class='btn btn-default margin-vert'><i class='fa fa-heart'></i> Donate to {$campaign->post_title}</button>";
                    $after .= "</form>";
                ...
                case 'about donations box':
                    ...
                    if(!$skip) {
                        ...
                        $desc .= "</div>";
                        $before_list .= '
                            <a href="#about-donation-'. $i .'" class="col-md-3 box-link action-block">
                                <div data-ratio="285:200" style="'. $styles .'" data-ratio-valign=".campaign-description">'. $desc .'</div>
                            </a>
                        ';
                        $before_list .= "<div id=\"{$id}\">";
                        $title = "<h2>{$campaign->post_title}</h2>";
            ...
            $output .= "<div {$id_attr_str}class='{$class_str}' rel='{$campaign->ID}'>";
            $output .= $before;
            if($description)
            {
